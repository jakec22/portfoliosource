...
/* Components                                                          */
/* ------------------------------------------------------------------ */
function MacroRing({ current, goal, label, color, unit = 'g', size = 80, strokeWidth = 8, }) {
    const radius = (size - strokeWidth) / 2;
    const circumference = 2 * Math.PI * radius;
    const progress = Math.min(current / goal, 1);
    const strokeDashoffset = circumference * (1 - progress);
    const over = current > goal;
    return (<View style={[ringStyles.container, { width: size, height: size + 30 }]}>
      <Svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        <Circle cx={size / 2} cy={size / 2} r={radius} stroke="#E5E7EB" strokeWidth={strokeWidth} fill="none"/>
        <Circle cx={size / 2} cy={size / 2} r={radius} stroke={over ? '#EF4444' : color} strokeWidth={strokeWidth} fill="none" strokeDasharray={circumference} strokeDashoffset={strokeDashoffset} strokeLinecap="round" rotation="-90" origin={`${size / 2}, ${size / 2}`}/>
      </Svg>
...
function CalorieSummary({ consumed, goal, size = 160, }) {
    ...
    return (<View style={calStyles.wrapper}>
      <View style={[calStyles.container, { width: size, height: size }]}>
        <Svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
          ...
          <Circle cx={size / 2} cy={size / 2} r={radius} stroke={over ? '#EF4444' : '#10B981'} strokeWidth={strokeWidth} fill="none" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" rotation="-90" origin={`${size / 2}, ${size / 2}`}/>
        </Svg>
...
function MealSection({ meal, date, onAddFood, }) {
    const [expanded, setExpanded] = useState(true);
    const entries = useStore((s) => s.getEntriesForDate(date)).filter((e) => e.meal === meal);
    const removeEntry = useStore((s) => s.removeEntry);
    const total = entries.reduce((acc, e) => acc + e.food.macros.calories * e.servings, 0);
    ...
    return (<View style={mealStyles.container}>
      ...
      {expanded && (<View style={mealStyles.body}>
          {entries.length === 0 ? (<Text style={mealStyles.empty}>No foods logged yet</Text>) : (entries.map((entry) => (<TouchableOpacity key={entry.id} style={mealStyles.entryRow} onLongPress={() => handleDelete(entry)} activeOpacity={0.7}>
    ...
    return (<SafeAreaView style={homeStyles.safe}>
      ...
      <ScrollView style={homeStyles.scroll} contentContainerStyle={homeStyles.content}>
        <View style={homeStyles.dateNav}>
          ...
          <TouchableOpacity onPress={() => goDay(1)} style={[homeStyles.navBtn, isToday && homeStyles.navBtnDisabled]} disabled={isToday}>
        ...
        <View style={homeStyles.card}>
          ...
          <View style={homeStyles.waterDots}>
            {[250, 250, 250, 250, 250, 250, 250, 250].map((ml, i) => (<TouchableOpacity key={i} onPress={() => addWater(selectedDate, ml)} style={[homeStyles.waterDot, water >= ml * (i + 1) && homeStyles.waterDotFilled]}>
        ...
        {['breakfast', 'lunch', 'dinner', 'snacks'].map((meal) => (<MealSection key={meal} meal={meal} date={selectedDate} onAddFood={handleAddFood}/>))}
      </ScrollView>
    </SafeAreaView>);
}
    ...
    return (<SafeAreaView style={logStyles.safe}>
      <KeyboardAvoidingView style={logStyles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <View style={logStyles.searchBar}>
          ...
          <TextInput style={logStyles.searchInput} placeholder="Search foods..." value={query} onChangeText={setQuery} autoCapitalize="none" clearButtonMode="while-editing" placeholderTextColor="#9CA3AF"/>
        </View>

        <FlatList data={results} keyExtractor={(f) => f.id} keyboardShouldPersistTaps="handled" contentContainerStyle={logStyles.listContent} renderItem={({ item }) => (<TouchableOpacity style={[logStyles.foodItem, selectedFood?.id === item.id && logStyles.foodItemSelected]} onPress={() => handleSelectFood(item)} activeOpacity={0.7}>
        ...
        {selectedFood && (<View style={logStyles.addPanel}>
            ...
            <View style={logStyles.servingsRow}>
              ...
              <View style={logStyles.servingsControl}>
                <TouchableOpacity style={logStyles.servingsBtn} onPress={() => {
                const v = Math.max(0.5, (parseFloat(servings) || 1) - 0.5);
                setServings(v.toString());
            }}>
                  <Text style={logStyles.servingsBtnText}>−</Text>
                </TouchableOpacity>
                <TextInput style={logStyles.servingsInput} value={servings} onChangeText={setServings} keyboardType="decimal-pad" selectTextOnFocus/>
                <TouchableOpacity style={logStyles.servingsBtn} onPress={() => {
            ...
            <TouchableOpacity style={logStyles.addBtn} onPress={handleAdd}>
              <Text style={logStyles.addBtnText}>Add to {MEAL_LABELS[meal]}</Text>
            </TouchableOpacity>
          </View>)}
...
function HistoryScreen() {
    ...
    function MacroBar({ value, goal, color }) {
        ...
        return (<View style={histBarStyles.track}>
        <View style={[histBarStyles.fill, { width: `${pct * 100}%`, backgroundColor: color }]}/>
      </View>);
    }
    return (<SafeAreaView style={histStyles.safe}>
      <ScrollView contentContainerStyle={histStyles.content}>
        ...
        {days.map((date) => {
            ...
            return (<View key={date} style={histStyles.dayCard}>
              <View style={histStyles.dayHeader}>
                <Text style={histStyles.dayLabel}>{displayDate(date)}</Text>
                {hasData ? (<Text style={[histStyles.dayCalories, totals.calories > goals.calories && histStyles.overGoal]}>
                    {Math.round(totals.calories)} / {goals.calories} kcal
                  </Text>) : (<Text style={histStyles.noData}>No data</Text>)}
              </View>
...
function SettingsScreen() {
    ...
    function GoalField({ label, field, unit, color, }) {
        return (<View style={setStyles.fieldRow}>
        ...
        <View style={setStyles.fieldInputWrap}>
          <TextInput style={setStyles.fieldInput} value={form[field]} onChangeText={(v) => setForm((f) => ({ ...f, [field]: v }))} keyboardType="number-pad" selectTextOnFocus/>
          <Text style={setStyles.fieldUnit}>{unit}</Text>
        </View>
      </View>);
    }
    const totalCalsFromMacros = parseInt(form.protein || '0') * 4 +
        parseInt(form.carbs || '0') * 4 +
        parseInt(form.fat || '0') * 9;
    return (<SafeAreaView style={setStyles.safe}>
      <ScrollView contentContainerStyle={setStyles.content}>
        ...
        <View style={setStyles.presetsGrid}>
          {PRESETS.map((preset) => (<TouchableOpacity key={preset.label} style={setStyles.presetCard} onPress={() => applyPreset(preset)} activeOpacity={0.7}>
        ...
        <View style={setStyles.card}>
          {[
            ...
            return (<View key={label} style={setStyles.distRow}>
                <Text style={[setStyles.distLabel, { color }]}>{label}</Text>
                <View style={setStyles.distBarTrack}>
                  <View style={[setStyles.distBarFill, { width: `${pct}%`, backgroundColor: color }]}/>
                </View>
